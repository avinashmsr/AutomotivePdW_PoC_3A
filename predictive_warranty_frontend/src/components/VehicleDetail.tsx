...
const DriverIcon: React.FC<{ kind: DriverKind }> = ({ kind }) => {
  ...
  switch (kind) {
    case "mileage":
      symbol = "🏎️"; // odometer / speed feel
      break;
    case "age":
      symbol = "⏱️"; // time in field
      break;
    case "engineTemp":
      symbol = "🔥"; // hot engine
      break;
    case "vibration":
      symbol = "📈"; // waveform / vibration
      break;
    case "services":
      symbol = "🛠️"; // service bay
      break;
    case "supplierPlant":
      symbol = "🏭"; // factory / plant
      break;
  }
...
const VehicleDetail: React.FC<VehicleDetailProps> = ({
  ...
  const rawRiskScore = vehicleDetail?.summary.risk_score ?? 0;
  const safeRiskScore = Math.min(1, Math.max(0, rawRiskScore));
  const riskBucket = vehicleDetail?.summary.risk_bucket ?? "Low";

  // Map 0–1 to -110° (left) … +110° (right)
  const needleAngle = -110 + safeRiskScore * 220;

  const needleBucketClass =
    riskBucket === "High"
      ? "needle-high"
      : riskBucket === "Medium"
      ? "needle-medium"
    ...
    <>
      ...
      {vehicleDetail && !loading && (
        <div className="detail-layout">
          ...
          {/* 2. KEY DRIVERS CARD (NOW BELOW THE GAUGE, MORE SPACE) */}
